...
func TestLogoutUserNormal(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			requestUser:    &model.User{Name: "logout_user", Email: "logout_user@example.com", Password: "password", Role: enum.AppUser},
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			req, _ := http.NewRequest("DELETE", "/user/logout", nil)
...
func TestLogoutUserAbnormal(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	testCases := []struct {
	...
	}{
		{
			...
			requestUser:    &model.User{Name: "invalid_token_user", Email: "invalid_token_user@example.com", Password: "password", Role: enum.AppUser},
			token:          "invalid_token",
		},
		{
			name:           "トークンなし",
			...
			errorCode:      "C-007",
			errorMessage:   "認証情報が切れています。再度ログインしてください",
			requestUser:    &model.User{Name: "no_token_user", Email: "no_token_user@example.com", Password: "password", Role: enum.AppUser},
			token:          "",
		},
	}
